...
export const createUserWithoutConfirmation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (createError) {
      ...
      const errorMsg = createError?.message?.toString() || "";

      if (/duplicate|cpf|já.*cadast|already.*regis|existed/i.test(errorMsg)) {
        return { success: false, alreadyRegistered: true, error: "Este CPF ou e-mail já está cadastrado. Faça login." };
      }

      return { success: false, error: errorMsg || "Erro ao conectar ao servidor. Tente novamente mais tarde." };
    }
