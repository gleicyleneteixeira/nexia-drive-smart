  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === "signup") {
        ...
        // Verificar se CPF já existe antes de tentar criar
        try {
          const { checkCpfExists } = await import("@/lib/admin-operations.server");
          const cpfCheck = await checkCpfExists({ data: { cpf } });
          if (cpfCheck.exists) {
            throw new Error("Este CPF já está cadastrado no sistema. Faça login.");
          }
        } catch (cpfErr) {
          if (cpfErr instanceof Error && cpfErr.message.includes("já está cadastrado")) {
            throw cpfErr;
          }
          console.error("Erro ao verificar CPF existente:", cpfErr);
        }
        ...
        if (!createRes.success) {
          const errorText = String(createRes.error || "Erro desconhecido");
          if (createRes.alreadyRegistered) {
            throw new Error(errorText || "Este CPF ou e-mail já está cadastrado. Faça login.");
          }
      ...
      const finalMsg = msg.length > 0 ? msg : "Erro ao processar sua solicitação. Tente novamente.";
      ...
      <div className="glass rounded-3xl p-7 shadow-card border border-border/40">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div>
            ...
            {mode === "login" && (
              <button type="button" onClick={() => { setForgotEmail(email); setForgotDone(null); setForgotOpen(true); }}
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {forgotDone
                ...
                : "Informe os dados do seu cadastro. Vamos mandar uma senha temporária no seu WhatsApp."}
            </DialogDescription>
          </DialogHeader>
          {forgotDone ? (
            <div className="space-y-4">
              <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-sm">
                {forgotDone === "whatsapp" ? (
                  ...
                  <>
                    📧 <strong>Confira seu e-mail!</strong> Não foi possível mandar no WhatsApp, então enviamos sua senha temporária para o e-mail do cadastro. Entre com ela e crie uma nova senha em seguida.
                  </>
                )}
              </div>
              <DialogFooter>
